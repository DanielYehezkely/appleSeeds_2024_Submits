...
class CarAgencyManager {
  ...
  _findAgencyById(agencyId) {
    return this.agencies.find(agency => agency.agencyId === agencyId) || null;
  }

  _findCarByCarNumber(agency, carNumber) {
    const cars = agency.cars;
    for (let i = 0; i < cars.length; i++) {
      const car = cars[i];
      for (const prop in car) {
        if (prop === 'models') {
          const models = car[prop];
          for (let j = 0; j < models.length; j++) {
            const model = models[j];
            if (model.carNumber === carNumber) {
              return { carIndex: i, modelIndex: j, model };
            }
          }
        }
      }
  ...
  searchAgency(idOrName) {
    return this.agencies.find(agency => agency.agencyName === idOrName || agency.agencyId === idOrName) || null;
  }
  ...
  changeAgencyCashOrCredit(agencyId, cashOrCredit) {
    ...
    if (agency) {
      if (cashOrCredit >= 0) {
        ...
        const totalCashOrCredit = Math.abs(cashOrCredit);
        if (agency.cash >= totalCashOrCredit) {
          agency.cash -= totalCashOrCredit;
        } else {
          const remainingCredit = totalCashOrCredit - agency.cash;
          agency.cash = 0;
          agency.credit -= remainingCredit;
        }
      }
      return true;
    }
...
console.log('Remove Car from Agency: ', carAgencyManager.removeCarFromAgency('Plyq5M5AZ', 'AZJZ4'));
...
// // // Test updateCarPrice
console.log('Update Car Price: ', carAgencyManager.updateCarPrice('Plyq5M5AZ', '6rvXw', 150000));
